...
public final class BentoCluster {
  private static final String HBASE_SITE_RELATIVE_PATH = "hbase/hbase-site.xml";
  private static final String CORE_SITE_RELATIVE_PATH = "hadoop/core-site.xml";
  private static final String MAPRED_SITE_RELATIVE_PATH = "hadoop/mapred-site.xml";
  private static final String YARN_SITE_RELATIVE_PATH = "hadoop/yarn-site.xml";
  ...
  private static final String[] SITE_FILE_RELATIVE_PATHS = new String[] {
      HBASE_SITE_RELATIVE_PATH,
      CORE_SITE_RELATIVE_PATH,
      MAPRED_SITE_RELATIVE_PATH,
      YARN_SITE_RELATIVE_PATH,
  };
  public static final String BENTO_NAME_PROPERTY = "bento.name";
  public static final String BENTO_NAME_FILE = "bento.properties";
   ...
   *     executable.
   * @throws java.lang.InterruptedException if the start bento cluster thread is interrupted.
   */
  public void start(
    ...
    // Add a command to create the user's home directory on hdfs.
    createHdfsUserHome(configDir);
  ...
  /**
   * Creates the user's home directory on hdfs. Assumes the hdfs namenode was started by the 'hdfs'
   * user. Assigns the resulting home directory to the 'supergroup' user group.
   *
   * @param configDir containing hadoop/hbase configuration files generated by the bento cluster.
   * @return the path to the current user's resulting home directory.
   * @throws java.io.IOException if there is an error calling the bento executable.
   * @throws java.util.concurrent.ExecutionException if there is an error calling the bento
   *     executable.
   * @throws java.lang.InterruptedException if the current thread is interrupted.
   */
  public Path createHdfsUserHome(
      final File configDir
  ) throws IOException, ExecutionException, InterruptedException {
    // Ensure bento cluster is running.
    Preconditions.checkState(isRunning(), "Bento cluster is not running!");

    final UserGroupInformation currentUser = UserGroupInformation.getCurrentUser();
    final UserGroupInformation hdfsUser = UserGroupInformation.createRemoteUser("hdfs");
    return hdfsUser.doAs(
        new PrivilegedExceptionAction<Path>() {
          @Override
          public Path run() throws URISyntaxException, IOException {
            // Get the hdfs configuration for the started bento cluster.
            final Configuration hadoopConf = new Configuration(false);
            hadoopConf.addResource(new File(configDir, CORE_SITE_RELATIVE_PATH).getAbsolutePath());

            // Create the user's hdfs home directory.
            final URI bentoHdfsUri = new URI(String.format("hdfs://%s:8020/", mBentoName));
            final FileSystem hdfs = FileSystem.get(bentoHdfsUri, hadoopConf);

            final Path bentoHdfsHome =
                new Path(bentoHdfsUri.resolve("user/").resolve(currentUser.getUserName()));
            hdfs.mkdirs(bentoHdfsHome);
            hdfs.setOwner(bentoHdfsHome, currentUser.getUserName(), "supergroup");

            return bentoHdfsHome;
          }
        }
    );
  }
   ...
   * @throws IOException if copying the site file or writing to the index file fails.
   */
  ...
  private void copySiteFile(
    ...
    final File outputFile = new File(pluginConfigDir, generatedSiteFile.getName());
    Files.copy(generatedSiteFile, outputFile);
    Preconditions.checkArgument(outputFile.exists());
    mMavenLog.debug("Wrote config site file: " + outputFile.getAbsolutePath());
    ...
    Files.append(outputFile.getAbsolutePath() + "\n", confIndexFile, Charsets.UTF_8);
    mMavenLog.debug("Appended site file path to conf index: " + confIndexFile.getAbsolutePath());
  }

  /**
   * Writes the name of the current bento to the specified bento output config directory.
   *
   * @param bentoConfigDir to store the bento name in.
   * @throws IOException if the write fails.
   */
  public void copyBentoName(final File bentoConfigDir) throws IOException {
    final Properties bentoProperties = new Properties();
    bentoProperties.setProperty(BENTO_NAME_PROPERTY, mBentoName);

    final PrintWriter printWriter = new PrintWriter(
        new File(bentoConfigDir, BENTO_NAME_FILE),
        Charsets.UTF_8.name()
    );
    try {
      bentoProperties.store(printWriter, "Properties relevant to a bento cluster.");
    } finally {
      printWriter.flush();
      printWriter.close();
    }
  ...
  private ShellResult executeAndLogCommands(
    ...
    // Log stdout/stderr.
    mMavenLog.debug("Stdout:");
    mMavenLog.debug(result.getStdout());
    mMavenLog.debug("Stderr:");
    mMavenLog.debug(result.getStderr());
    mMavenLog.debug(String.format("Exit code: %d", result.getExitCode()));
